...
This skill ships inside **[`developing-today/hardware-doc`](https://github.com/developing-today/hardware-doc)**, a standalone repository that *is* the hardware knowledge base. Paths below are relative to its root.

```text
hardware-doc/                        ← repository root
├── README.md
├── AGENTS.md                        # working conventions
├── ai-crawler-site-access-table.md  # per-site retrieval findings
...
└── shared-assets/                   # symlink pool for byte-identical duplicates
```

Use lowercase, filesystem-safe slugs and exact manufacturer and part numbers. Put reusable standard interfaces under `components/generic/`. Scope unidentified parts to the device — `components/unidentified/<device-slug>/<function>/` — so unrelated unknown parts never collapse into one record.
...
`` is not a closed taxonomy. If material genuinely fits none of `devices/`, `components/`, `vendors/` or `guides/`, add a category rather than forcing a bad fit — but only when the misfit is real, and say in the index why the category exists.
