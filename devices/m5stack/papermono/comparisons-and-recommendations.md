...
|---|---|---|
| **W3 — you need NFC *and* LoRa *and* e-paper on one battery-powered board** (access control, identity terminal, ticketing) | **Nothing else found in this survey does all three.** ST25R3916 covers ISO14443A/B, FeliCa and ISO15693; SX1262 covers 868–923 MHz. M5Stack's FCC-filed manual names exactly this use case first: *"access control systems, identity authentication"*, and the device is SAR-certified at **0 mm body contact on all six faces** — the posture of a badge or a reader held against a surface | **Revised 2026-09-20: US paperwork now exists** (FCC ID `2AN3WM5PAPERMONO`). LoRa is authorised **903.0–914.9 MHz only** at **11.5 dBm** into a **−5 dBi** antenna, so the sub-GHz link is short. Outside the US there is still no located paperwork. Which card types actually interoperate is **untested by anyone** |
...
| **You need regulatory paperwork** | **Revised 2026-09-20.** `/en/certification` really does return zero matches — but the board **is** certified: **FCC ID `2AN3WM5PAPERMONO`**, granted 2026-08-12, with a Japanese MIC number on the label. You can ship this into the US. **You cannot rely on it elsewhere**: no CE DoC, IC, RCM, UKCA or SRRC record has been located, and the US LoRa grant covers **903.0–914.9 MHz only** | For the US this is no longer a reason to reject the board. For the EU/UK/AU, a vendor that publishes a DoC. The PaperMono now has its own write-up: [`certification.md`](certification.md), comparable to the Inkplate [regulatory and SKU record](../../soldered-electronics/inkplate-5/regulatory-and-skus.md) |
...
| **Professional / regulated deployment** | **PaperMono, for the US only** | — | **Revised 2026-09-20.** The PaperMono is the one device in this comparison whose certification has now been read end to end ([`certification.md`](certification.md)) — FCC only, LoRa restricted to 903.0–914.9 MHz. For every other device here, certification remains unverified in this pass, and for every non-US market the PaperMono's status is unknown too. Not a recommendation to proceed outside the US |
