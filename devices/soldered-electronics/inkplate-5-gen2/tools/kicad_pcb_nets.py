...
for m in re.finditer(r"\(footprint ", src):
    ...
    if not ref:
        # KiCad 6 (file version 20211014) stores the designator as fp_text, not property
        ref = re.search(r'\(fp_text reference "([^"]*)"', blk)
    ref = ref.group(1) if ref else "?"
    val = re.search(r'\(property "Value" "([^"]*)"', blk)
    if not val:
        val = re.search(r'\(fp_text value "([^"]*)"', blk)
    ...
    for p in re.finditer(r'\(pad "([^"]*)"[^\n]*', blk):
        ...
        if nm:
            nets[nm.group(1)].append((ref, p.group(1), val))
arg = sys.argv[2] if len(sys.argv) > 2 else None
if arg and arg != "--all":
    rows = [(n, r, pad) for n, pins in nets.items() for r, pad, _ in pins if r == arg]
    for n, r, pad in sorted(
        rows, key=lambda x: (int(x[2]) if x[2].isdigit() else 999, x[2])
    ):
        print(f"{r}.{pad:<5} -> {n}")
    print(f"\n{len(rows)} pads")
else:
