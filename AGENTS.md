...
## Pins, hashes and identifiers are append-only

**Never overwrite or delete a commit SHA, content hash, URL, version or artifact
identifier that is already recorded. Add the new one; mark the old deprecated.**

This applies to `examples/catalog.json`, `artifact-manifest.md`, every
`*.ARCHIVED.md` placeholder, `sources.md` tables, and any inline `@ <sha>`
citation in prose.

### Why

A recorded pin is evidence for a claim made *at that ref*. If you replace
`67ee512` with today's HEAD, every sentence in the tree that was written from
`67ee512` silently becomes unsourced — and nothing flags it. The old ref usually
still resolves on the host, and if the repo is later deleted, renamed or
force-pushed, the superseded SHA is the only thing that lets a future reader find
it in a fork or an archive.

This was learned the expensive way: a 2026-09-11 enrichment pass overwrote three
pins with current HEADs, breaking the link between three device-record findings
and the code they were actually read from.

### The rule, concretely

| Situation | Do |
|---|---|
| Upstream moved | Add the new SHA to `commit`; push the old into `prior_refs[]` |
| A claim was read at an older ref | Set `analysed_at_commit` and say so |
| A URL 404s | Keep it, add `status: dead` and the date; add the replacement alongside |
| A project is renamed | Keep the old `canonical_url` as a `prior_refs[]` entry |
| A hash changes | **Stop.** A changed hash for the same name+version is itself a finding — record both |
| An entry is incomplete | Keep it. Mark the unknown fields `null` with a `_note_*` explaining why |

**Incomplete entries are never deleted for being incomplete.** An entry with three
verified fields is strictly better than no entry, because it records that the
thing exists.

### Catalog schema for this

```jsonc
"commit":            "06e1e5ed…",   // current pin
"commit_date":       "2026-09-20",
"commit_subject":    "Merge pull request #1 …",
"analysed_at_commit":"67ee512024de", // what our prose was actually read from
"analysed_at_note":  "⚠ documentation claims were read at this earlier ref …",
"prior_refs": [
  { "commit": "67ee512024de", "date": "2026-09-06", "subject": "docs: audit …",
    "provenance": "handoff 03-new-projects.md, 2026-09-11",
    "resolves": true, "verified": "2026-09-11",
    "url": "https://github.com/…/tree/67ee512024de" }
]
```

Set `resolves` by actually checking (`GET /repos/{o}/{r}/commits/{sha}`), and
record the date you checked. `resolves: false` is valuable — it means the ref is
gone and only a fork or the Wayback Machine will have it.

### Check before you commit

```bash
./scripts/check-pins.py            # fails if a pin was dropped or rewritten
```

It compares the working tree against `HEAD` and reports any SHA, hash or URL that
was **removed or changed in place** rather than appended.
