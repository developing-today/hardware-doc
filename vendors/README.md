...
| **Solomon Systech** | [solomon-systech/](solomon-systech/README.md) | [SSD1677](../components/solomon-systech/ssd1677/README.md) · [SSD1680](../components/solomon-systech/ssd1680/README.md) · [SSD1681](../components/solomon-systech/ssd1681/README.md) · [SSD1683](../components/solomon-systech/ssd1683/README.md) · [SSD7317](../components/solomon-systech/ssd7317/README.md). ⚠ **Mirror-sourced PDFs from this vendor arrive misnamed** — two of five filed in one pass had the wrong part in the filename and a third had the wrong part in its own `Title` metadata. **Verify by first-page text, never by filename.** Covers the **Crystalfontz embedded-JSON catalogue** (the only way to enumerate what exists, with a live-verified id table), the **decorative-URL trap** (`/controllers/<Vendor>/<Part>/<id>/` — only `<id>` selects a document, and a wrong id returns a different part with HTTP 200 and valid `%PDF` magic), a **footer-census recipe** that yields part, revision, date, page-integrity *and* multi-document-bundle detection in one command, and ⚠ **why "SSD16xx" is not one command set** (`0x32` takes 105/153/227 bytes depending on part; RAM X is addressed in pixels on one member and bytes on the rest). ⚠ **The SSD1677 — the only family member fitted on hardware here — is absent from Crystalfontz entirely.** *Added 2026-09-20* |
