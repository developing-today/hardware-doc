...
> **This audit is the worked example; the rule lives in the method.**
> [`SKILL.md` § *When to archive*](.agents/skills/hardware-device-research/SKILL.md#when-to-archive)
> states the test an artifact must pass before it may be moved out — content already mined into
> Markdown, genuinely derived/bulky/off-topic, and a placeholder able to carry enough provenance to
> reacquire it. **Size alone is not a reason to archive; *derivedness* is.**
> This page applies that test to the real tree, category by category, so a future pass can usually
> read off the answer instead of re-deriving it. If the two ever disagree, the method wins and this
> page needs re-measuring.
