...
K230 **MMC1**: `CMD` `GPIO54`, `CLK` `GPIO55`, `D0`–`D3` `GPIO56`–`GPIO59`, 4-bit.

**The card is the boot medium *and* the root filesystem** — there is no on-board storage the
system boots from, so card quality is system reliability and removal while running pulls the
rootfs. BSP v0.2.4 added an SD-expansion step for cards larger than the flashed image; before
