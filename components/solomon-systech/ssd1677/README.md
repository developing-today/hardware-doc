...
- **Research status:** the **full datasheet is retained** in `artifacts/` and has a valid text layer, cross-checked against rendered headings. Board-side behaviour is unusually well evidenced because M5Stack ships two independent drivers for the same panel and they disagree with each other in instructive ways. *Updated 2026-09-20:* a **second, byte-different copy** of the same document was compared and retained (§12), which resolved §7-C8 — one page **was** dropped, and it is the revision-history page.
- **Retrieved:** 2026-09-01; artifact and caveat update 2026-09-20
...
| **C5** | **Deep-sleep parameter `0x01` is not a documented value — ⚠ but it is on every sibling.** This datasheet's table defines only `00` (normal) and `11` (⇒ `0x03`). **Both** DKE's reference program and M5Stack's demo write `0x01`. **Updated 2026-09-20:** the [SSD1680](../ssd1680/README.md#66-deep-sleep--two-modes-and-the-ram-retention-difference), [SSD1681](../ssd1681/README.md#53-deep-sleep--two-modes) and [SSD1683](../ssd1683/README.md) datasheets all document **three** values for `0x10` — `00` Normal, **`01` Enter Deep Sleep Mode 1 (RAM retained but inaccessible)**, `11` Enter Deep Sleep Mode 2 (RAM not retained) — with separate current specifications for each. The SSD1683 goes further and prints `0x10` `{0x03}` as the literal parameter in its published deep-sleep flow. So `0x01` is a **real, documented family encoding**, and the far likelier explanation is that the SSD1677 silicon implements the same two modes and **this document omitted mode 1** — not that two vendors independently made the same typo. Still **`inferred`**: it is sibling-datasheet evidence, not SSD1677 evidence, and it is untested. It is also cheaply testable — on an SSD1677, `{0x01}` should preserve RAM across the sleep where `{0x03}` does not | **[DS]** §7 (`0x10`); **[MAN]** p. 24; **[SRC]** `EDP_OTP_LUT_demo.cpp:236`; sibling datasheets, §11 |
...
| **C8** | **⚠ RESOLVED 2026-09-20 — printed page 2 is missing, and it is the revision-history page.** The PDF has **46 pages** while every footer numbers `P n/47`. A page-by-page footer census (`executed-success`) now shows the mapping exactly: **PDF page 1 → footer `P 1/47`** (cover), **PDF page 2 → footer `P 3/47`** (CONTENTS), and PDF page *n* → footer *n+1* for all 45 remaining pages. So exactly **one** page was dropped and it is **printed page 2**. In all four sibling documents now held — [SSD1680](../ssd1680/README.md), [SSD1681](../ssd1681/README.md), [SSD1683](../ssd1683/README.md) and [SSD7317](../ssd7317/README.md) — **printed page 2 is the "Appendix: IC Revision history of SSD&lt;part&gt; Specification" table**, without exception. **[INF]**, but with four-for-four family corroboration: the missing page is this document's revision history. That is also why this record could only say Rev 1.0 is *"the first and, as far as this pass established, only public revision"* — **the page that would have said so was cut out.** ⚠ **The second copy acquired on 2026-09-20 (§12) is missing the same page**, so the loss happened upstream of both PDFill renderings, in a common ancestor. No §1–§13 body content is missing | `executed-success` — `pdfinfo` + footer census on both copies; sibling comparison **[DS1680/1681/1683]** |
...
- [Vendor sourcing guides index](../../../vendors/README.md) — **a [Solomon Systech sourcing guide now exists](../../../vendors/solomon-systech/README.md)** *(added 2026-09-20)*. ⚠ Read its §2 before fetching any Solomon Systech PDF: mirror-sourced copies of this vendor's documents arrive **misnamed**, and must be verified by first-page text rather than by filename. This datasheet was obtained **only** as an M5Stack product-page mirror; no Solomon Systech-hosted copy was located, and §12's negative result now also rules out Crystalfontz

### Sibling controllers — *added 2026-09-20*

Four further Solomon Systech display controllers are now documented here. **None is fitted on any device in this repository**; all are reference-only, and each record says why it was acquired.

| Record | What it is | Why it matters to the SSD1677 |
|---|---|---|
| [**SSD1680**](../ssd1680/README.md) | 176 × 296 EPD controller, Rev 0.14 | **Documents `0x0C` Booster Soft-start in full** — the bit table this part's datasheet omits. The source of the decode in [`papermono/gaps-and-conflicts.md`](../../../devices/m5stack/papermono/gaps-and-conflicts.md) |
| [**SSD1681**](../ssd1681/README.md) | 200 × 200 EPD controller, Rev 0.13 | Same `0x0C` table; the "familiar 153-byte LUT" confirmed from primary source; and a worked example of an inherited-defect datasheet |
| [**SSD1683**](../ssd1683/README.md) | 400 × 300 EPD controller, Rev 1.0 | Same `0x0C` table; **prints `0x10` `{0x03}` as a literal deep-sleep parameter**, informing §7-C5 |
| [**SSD7317**](../ssd7317/README.md) | ⚠ **OLED + in-cell touch — not e-paper** | Unrelated silicon. Filed under this vendor because it arrived in the same misnamed batch; its record is the worked example behind the sourcing guide's §2 |

⚠ **The family is not one command set.** `0x32` takes **105 bytes here, 153 on the SSD1680/1681, 227 on the SSD1683**; this part addresses RAM X in **pixels** while the others address it in **bytes**; the waveform engine is 40 phases here, 48 there, 32 on the SSD1683. Do not port a LUT or a windowing routine between them.
...
|---|---:|---|---|
| `artifacts/ssd1677-rev1.0-nov2018-m5stack-mirror.pdf` | 3 602 487 | `daf8b726f822f6907529386b8af6fbea6ec5e8ecc7b7af361340be911d632544` | **SSD1677 datasheet, "Advance Information", Rev 1.0, Nov 2018.** PDF 1.4, A4, **46 pages** (document numbers itself `n/47` — §7-C8). Producer **`PDFill: Free PDF Writer and Tools`**, CreationDate **= ModDate = 2019-01-09 00:44:32 CST** — a re-written copy, not Solomon Systech's own export (§7-C9) |
| `artifacts/ssd1677-rev1.0-nov2018-pdfill-2020-variant.pdf` | 2 576 883 | `4b5cc241f410722e54c4d756424907b90ed988502216ed5ed864dd0df78b7e0c` | **The same document, a different PDFill rendering** — *added 2026-09-20*. PDF 1.4, A4, **46 pages**, same missing printed page 2. Producer **`PDFill: Free PDF Writer and Tools`**, CreationDate **2019-01-08 02:09:12 CST** (≈22½ h *earlier* than the copy above), **ModDate 2020-04-02 22:08:23 CDT** — modified fifteen months later. **Provenance unknown**; see below |

### The two SSD1677 copies — compared 2026-09-20

A second copy of this datasheet arrived in a 2026-09-11 scratch batch under the filename `ssd1677_real.pdf`. It was compared against the copy already held, rather than assumed redundant:

| Property | `…m5stack-mirror.pdf` | `…pdfill-2020-variant.pdf` |
|---|---|---|
| **SHA-256** | `daf8b726…` | **`4b5cc241…`** — **different** |
| Bytes | 3 602 487 | **2 576 883** (−28.5 %) |
| Pages | 46 | 46 |
| Producer | PDFill: Free PDF Writer and Tools | PDFill: Free PDF Writer and Tools |
| CreationDate | 2019-01-09 00:44:32 CST | **2019-01-08 02:09:12 CST** |
| ModDate | 2019-01-09 00:44:32 CST | **2020-04-02 22:08:23 CDT** |
| Metadata Stream | no | **yes** |
| Printed page 2 | **missing** | **missing** |

**Verdict: the same document, two different renderings — not two revisions.** Evidence, all `executed-success`:

- Both are **Rev 1.0, Nov 2018, "Advance Information"**, 46 pages, with the identical footer sequence and the identical missing printed page 2 (§7-C8).
- Whitespace-normalised `pdftotext -layout` output differs in **exactly two places across 2 360 lines**: one table cell wraps onto a second line (`SSD1677Z | Gold Bump Die | Die thickness: 300um` versus the same content split across two rows) and one blank line is present in one and not the other. **Every value, every register, every table entry is identical.** No content difference exists.
- Neither is Solomon Systech's own export; **both** passed through *PDFill: Free PDF Writer and Tools*, ~22 hours apart in January 2019.

**Both are retained**, per the rule that a differing SHA-256 under one version is itself a finding. The concrete value of keeping the second copy is that it **independently corroborates §7-C8**: the missing revision-history page is not an artefact of M5Stack's mirroring, because a copy that never went near M5Stack is missing the same page. The loss is upstream of both, in whatever produced the January 2019 PDFill source.

⚠ **Provenance of the second copy is unrecorded.** The `_real` suffix on its scratch filename shows the acquiring session had just been burned by a misnamed fetch and refetched deliberately — the file it was correcting turned out to be an **SSD1681** ([now filed](../ssd1681/README.md#10-local-artifacts)) — but **no URL was recorded and none was recovered.** It is **not** from Crystalfontz: that catalogue was probed live on 2026-09-20 and **contains no SSD1677 entry at all** (S-9). Recorded as `unknown` rather than guessed. The situation stated in §12 stands: **the M5Stack product page remains the only known acquisition URL for this datasheet.**
...
**Re-confirmed 2026-09-20, with a new negative result.** Crystalfontz's controller catalogue — which *does* carry the [SSD1680](../ssd1680/README.md), [SSD1681](../ssd1681/README.md), [SSD1683](../ssd1683/README.md), [SSD7317](../ssd7317/README.md) and an SSD1673 — **has no SSD1677 entry**. The Solomon Systech section of its embedded JSON catalogue was enumerated in full (`executed-success`, S-9): 33 entries, `SSD1289` through `SSD7317`, and `SSD1677` is not among them. **So the most productive third-party mirror for this vendor does not have this part.** That is a useful negative: it rules out the obvious next place to look, and it explains why a 2026-09-11 attempt to fetch "SSD1677" from that host returned a different document entirely (§12, and [`vendors/solomon-systech` §2](../../../vendors/solomon-systech/README.md)).
...
| S-8 | SSD1677 datasheet — second PDFill rendering | **Solomon Systech Limited**, mirror unknown | mirror, **source unrecorded** | datasheet | `artifacts/ssd1677-rev1.0-nov2018-pdfill-2020-variant.pdf` | 2026-09-11 (acquired), **2026-09-20** (compared, filed) | **Rev 1.0, Nov 2018 — "Advance Information"**; PDF CreationDate 2019-01-08, ModDate 2020-04-02 | §12 — content-identical second copy; independently corroborates the missing printed page 2 (§7-C8) |
| S-9 | Crystalfontz controller catalogue | Crystalfontz America | credible mirror | catalogue (embedded JSON) | `https://www.crystalfontz.com/controllers/` | **2026-09-20** | undated | §12 — **negative result**: the Solomon Systech section lists 33 parts and **contains no SSD1677**; it does carry SSD1673, SSD1680, SSD1681, SSD1683 and SSD7317 |
| S-10 | Sibling datasheets — SSD1680 Rev 0.14, SSD1681 Rev 0.13, SSD1683 Rev 1.0, SSD7317 Rev 1.3 | Solomon Systech | primary / credible mirror | datasheets | [`../ssd1680/`](../ssd1680/README.md) · [`../ssd1681/`](../ssd1681/README.md) · [`../ssd1683/`](../ssd1683/README.md) · [`../ssd7317/`](../ssd7317/README.md) | **2026-09-20** | various | §7-C5 (deep-sleep mode 1 is a documented family encoding), §7-C8 (printed page 2 is the revision history in all four) |
