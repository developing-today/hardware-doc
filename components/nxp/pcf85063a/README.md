...
- [LilyGO T-Display-S3 AMOLED Plus](../../../devices/lilygo/t-display-s3-amoled-plus/README.md) — designator `U7`, part string `PCF85063ATL/1`, backed by a **Seiko MS412FE-FL26E** rechargeable lithium coin cell at `U8` so the clock survives a main-battery swap. The only board in the T-Display-S3 family with a battery-backed RTC other than the round DS-series boards (which use a [PCF8563](../pcf8563/README.md) instead).
- [LilyGO T-Display-S3 Pro MVSRBoard](../../../devices/lilygo/t-display-s3-pro-mvsrboard/README.md) — the **R** in *MVSR*. LilyGO mirrors `PCF85063ATL-1,118.pdf` in the `T-Display-S3-Pro` repository @ `08b8b2b`; not retrieved by that pass, since this record already holds the part.
...
- [Waveshare ESP32-S3-ePaper-3.97](../../../devices/waveshare/esp32-s3-epaper-3.97/README.md) — *added 2026-09-01*. Waveshare names the part **`PCF85063`**; the schematic label is **`PCF85063ATL`** (TSSOP14). On the board's **single shared I²C bus** (GPIO41 SDA / GPIO42 SCL) together with an [AXP2101](../../x-powers/axp2101/README.md) PMIC at `0x34`, an [ES8311](../../everest-semiconductor/es8311/README.md) codec, an [SHTC3](../../sensirion/shtc3/README.md) at `0x70` and a [QMI8658A](../../qst/qmi8658a/README.md) IMU. Interrupt net `RTC_INT_ESP` → **GPIO45**. Backup power comes from a dedicated **PH1.25-2P coin-cell header `H4`** (net `VBACKUP`, via `R61` 0 Ω) which the **AXP2101 actively charges at 3.0 V** — so *fitting a non-rechargeable cell to that header would charge a primary cell*. The `0x51` address is the part's documented default and was **not** confirmed against this board's source (`inferred`)

### A second, staler copy of this datasheet — *added 2026-09-01*

Waveshare mirrors this datasheet from `files.waveshare.com/wiki/common/` and links it from the ESP32-S3-ePaper-3.97 documentation. Hashed against the NXP copy already held here:

| Copy | Bytes | SHA-256 | Revision |
|---|---:|---|---|
| NXP original | 726 288 | `a09223db…f430f6000b` | **Rev. 7, 30 March 2018** |
| [`artifacts/pcf85063a-datasheet-rev6-waveshare-mirror.pdf`](artifacts/pcf85063a-datasheet-rev6-waveshare-mirror.pdf) | 535 722 | `93b384873539e770305b8982bcfa6387eac695650063b6a11279beae7a20602a` | **Rev. 6, 2015** |

**Both retained**, per the method's rule that the same document at a different hash means keep both and investigate. Waveshare ships a copy **three years and one revision behind** the manufacturer's — a third independent confirmation of the "re-hosted third-party PDFs may be stale" warning in the [Waveshare sourcing guide](../../../vendors/waveshare/README.md). **Design against Rev. 7**; the mirror documents only what Waveshare hands its customers. Retrieved 2026-09-01, validated by magic bytes, licence `unknown`, disposition `repository`.
